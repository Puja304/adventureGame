...
void play::hallway_locations(){
    ...
    locations[3]->addDescription("You are outside room 3. You hear a horrid sound coming from the inside heat emanating from the door. Whatever is on the other side cannot be good.");
...
void play::rooms_and_items(){
    ...
    item sword("sword");
    sword.setInteract(item::foundSword);

    item painting("painting");
    sword.setInteract(item::paintingCode);
    
    locations[11] = new area{31,20,true, sword, desk, dresser};
    ...
    locations[11]->addDescription("You enter room 1. It is utterly silent as you walk in, and yet you can hear the phantom screams of everything that has"
    "happened here, vividly documented in the scratch marks and dragging across walls and splashes of rust red on the floor. Quite out of place is a painting on the main wall,"
    
    " what was probably a pristice white canvas to the painting of a horse, but was now splashed with the same red as the floor. To its right is a wall with "
    "metals cuffs attached, empty now but well-used. Right above the cuffs is a sword attached with a different set of cuffs. On its left is a large "
    "window overlooking a cement sidewalk.");
